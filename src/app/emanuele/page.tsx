...
const EXTRAS = [
    { href: "/emanuele/bonus", icon: Star, label: "Sfida Bonus", color: "text-yellow-400" },
];
            ...
            <div className="w-full">
                ...
                <div className="quest-card overflow-hidden mb-8 slide-up relative min-h-[300px] flex flex-col justify-end p-6 sm:p-10 border-emerald-500/30 shadow-[0_0_50px_rgba(16,185,129,0.15)] rounded-2xl">
                    ...
                    <div className="absolute top-4 right-4 z-20 flex items-center gap-2">
                        {prefix && <ExportButton prefix={prefix} filename="emanuele_game_designer_backup.json" />}
                        <Link href="/" className="btn-secondary text-xs py-2 px-4 bg-black/40 backdrop-blur-md hover:bg-black/60 border border-white/10 rounded-xl transition-all">⬅ Home</Link>
                    </div>
                    ...
                    <div className="relative z-10 w-full max-w-3xl">
                        <div className="breadcrumb mb-4 text-xs font-semibold opacity-80 flex items-center gap-2">
                            ...
                            <span className="separator">/</span>
                            <span className="text-white drop-shadow">Emanuele</span>
                        </div>
                        <h1 className="text-4xl sm:text-6xl font-black text-white drop-shadow-lg tracking-tight mb-3 flex items-center gap-4">
                            <Settings className="w-10 h-10 sm:w-12 sm:h-12 text-emerald-400" />
                            Laboratorio Game Design
                        </h1>
                        <p className="text-emerald-100 text-lg sm:text-xl drop-shadow-md font-medium max-w-xl">Progetta le meccaniche, bilancia le statistiche e scrivi le regole del gioco!</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 xl:gap-8">
                    ...
                    <div className="lg:col-span-2 flex flex-col gap-6">
                        {/* Progress Bar */}
                        {completed.length > 0 && (
                            <div className="quest-card p-6 slide-up rounded-2xl shadow-lg border border-emerald-500/20 bg-gradient-to-br from-black/40 to-emerald-900/10" style={{ animationDelay: "0.1s" }}>
                                <div className="flex justify-between items-center text-sm mb-4">
                                    ...
                                    <span className={`font-bold px-4 py-1.5 bg-black/50 rounded-full border ${allDone ? "border-emerald-500/50 text-emerald-400" : "border-emerald-500/30 text-emerald-400"} text-sm shadow-inner`}>
                                ...
                                {allDone && <p className="text-center text-emerald-400 text-sm mt-4 font-bold pop-in bg-emerald-500/10 py-2 rounded-lg border border-emerald-500/20">✨ Tutte le missioni completate! Sei un vero Game Designer!</p>}
                            </div>
                        )}

                        <div className="flex items-center gap-4 slide-up mt-2" style={{ animationDelay: "0.15s" }}>
                            <h2 className="text-2xl font-black text-white tracking-tight">Missioni Principali</h2>
                    ...
                    <div className="flex flex-col gap-6">
                                ...
                                {EXTRAS.map(e => {
                                    ...
                                    return (
                                        <Link key={e.href} href={e.href}
                                            ...
                                            <span className="text-white/20 group-hover:translate-x-1 group-hover:text-emerald-400 transition-all">➔</span>
                                        </Link>
                                    );
                                })}
                            </div>
                        ...
                        <div className="quest-card overflow-hidden slide-up relative group border-emerald-500/20" style={{ animationDelay: "0.5s" }}>
                            <div className="w-full h-[180px] relative bg-black/80">
                                <Image src="/assets/example_enemy_types.png" alt="Esempio Tipi di Nemici" fill className="object-cover opacity-60 group-hover:opacity-100 transition-opacity duration-500 group-hover:scale-105" />
                            ...
                            <div className="absolute bottom-4 left-4 right-4 z-10 flex flex-col items-center">
                                <h3 className="text-white font-black text-lg drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] text-center tracking-tight">Tipi di Nemici</h3>
                                <p className="text-xs text-emerald-400 font-bold bg-black/60 px-2 py-0.5 rounded-full mt-1 border border-white/10 backdrop-blur-sm shadow-lg">Reference</p>
                            </div>
                        </div>
                    </div>
                </div>
